...
describe('Icon Component', () => {
    it('render', () => {
        render(React.createElement(Icon, { iconSrc: IconSearch }));
        const icon = screen.getAllByRole('img')[0];
        expect(icon).toBeInTheDocument();
    });
    it('can set style', () => {
        render(React.createElement(Icon, { iconSrc: IconSearch, size: 24, border: '1px solid red', margin: '10px', padding: '13px' }));
    ...
    it('can set size', () => {
        render(React.createElement(Icon, { iconSrc: IconSearch, size: 24 }));
        const icon = screen.getAllByRole('img')[2];
        expect(icon).toHaveAttribute('width', '24');
        expect(icon).toHaveAttribute('height', '24');
    });
    it('can set cursor', () => {
        render(React.createElement(Icon, { iconSrc: IconSearch, size: 24, cursor: 'pointer' }));
